...
def build_model(X_train, y_train, X_test, y_test, num_epoch, batch_size):
    ...
    model = Sequential()
    model.add(Conv1D(32, strides=1, input_shape=(length, 1), activation=activation, kernel_size=3, padding='same'))
    model.add(BatchNormalization())
    model.add(Conv1D(32, strides=1, activation=activation, kernel_size=3, padding='same'))
    model.add(MaxPooling1D(2, strides=2, padding='same'))
    model.add(BatchNormalization())
    model.add(Conv1D(64, strides=1, activation=activation, kernel_size=3, padding='same'))
    model.add(BatchNormalization())
    model.add(Conv1D(64, strides=1, activation=activation, kernel_size=3, padding='same'))
    model.add(MaxPooling1D(2, strides=2, padding='same'))
    model.add(Conv1D(128, strides=1, activation=activation, kernel_size=3, padding='same'))
    model.add(Conv1D(64, strides=1, activation=activation, kernel_size=3, padding='same'))
